            ...
            <div class="card">
                ...
                <div class="card-body">
                     ...
                     <div class="row mb-2">
                       ...
                       <form autocomplete="off" action="{{ route('payments.update',$payment->id) }}" method="Post">
                        ...
                        <div class="row">
                                    ...
                                    <label>Customers <span class="text-danger"></span></label>
                                     <select class="form-control" name="customer">
                                        ...
                                        @foreach($customers as $customer)
                                         <option value="{{$customer->id}}" @selected($customer->id==$payment->customer)>{{$customer->name}}</option>   
                                        @endforeach
                                      </select>
                            </div>
                            ...
                            <div class="form-group col-lg-2 mt-2">
                                <label>Amount <span class="text-danger">*</span></label>
                                   <input type="number" class="form-control" name="amount" id="amount" value="{{$payment->amount}}" placeholder="Enter Amount" oninput="calculateConvertedAmount()" required/>
                            </div>
                            ...
                            <div class="form-group col-lg-2 mt-2">
                                ...
                                <select class="form-control" name="currency" id="from_currency" oninput="calculateConvertedAmount()" required>
                                    ...
                                    <option value="usd" @selected('usd' == $payment->currency)>($) USD</option>
                                    <option value="eur" @selected('eur' == $payment->currency)>(€) EURO</option>
                                    <option value="gbp" @selected('gbp' == $payment->currency)>(£) GBP</option>
                                    <option value="inr" @selected('inr' == $payment->currency)>(₹) INR</option>
                                    <option value="aud" @selected('aud' == $payment->currency)>($) AUD</option>
                            ...
                            <div class="form-group col-lg-2 mt-2">
                                ...
                                <select class="form-control" name="crypto" id="to_crypto" oninput="calculateConvertedAmount()" required>
                                    <option value="tether" @selected('tether' == $payment->crypto)>Tether USDT</option>
                                    <option value="bitcoin" @selected('bitcoin' == $payment->crypto)>Bitcoin</option>
                                    <option value="ethereum" @selected('ethereum' == $payment->crypto)>Ethereum</option>
                                    <option value="usdc" @selected('usdc' == $payment->crypto)>USDC</option>

                                    <option value="dogecoin" @selected('dogecoin' == $payment->crypto)>Dogecoin</option>
                                    <option value="solana" @selected('solana' == $payment->crypto)>Solana</option>

                                    <option value="tron" @selected('tron' == $payment->crypto)>Tron</option>
                                    
                                    <option value="polygon" @selected('polygon' == $payment->crypto)>Polygon</option>
                            ...
                            <div class="form-group col-lg-3 mt-2">
                                ...
                                <input type="number" class="form-control" id="convert_amount" name="convert_amount" placeholder="Enter Amount" value="{{$payment->convert_amount}}" readonly>
                            </div>
