    ...
    <div class="container">
        ...
        <div class="row">
            ...
            <div class="col-lg-8">
                ...
                <h6>PayIT123CRM Processing Client - <span style="color: yellow;">Queries on Additional Responses</span></h6>
                @include('flash_msg')
                <form action="{{route('kycrequests.recordResponse')}}" method="Post">
                    @csrf
                    <input type="hidden" name="eddID" value="{{$EddValues->id}}">
                    <div class="row mt-4">
                        <h3 class="head-h3">Greetings <span>{{$EddValues->onboardDetail->company_name}} </span>!</h3>
                        <p>The below queries are raised based on the responsed received from you through our Additional Questionnaire. <br>
                            Please provide clarifications for these queries too.</p>
                    ...
                    <div class="row">
                        <div class="col-md-12 row">
                            ...
                            <div class="col-lg-12 mt-4">
                                @if($EddValues->status !== 'Response Received')
                                @if($questions)
                                @foreach($questions as $qs)
                                @if($qs->status === 'Open')
                                <div class="mb-3">

                                    <label for="exampleFormControlInput1" class="form-label mb-3"><span style="color: yellow !important;">Q: </span> {{$qs->question}}</label>
                                    <input type="text" class="form-control" id="exampleFormControlInput1" name="answers[]" placeholder="Your clarification here..." required>
                                    <input type="hidden" name="questionIDs[]" value="{{$qs->id}}">
